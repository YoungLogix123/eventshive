...
class FunctionsV3
{
	...
	public static function getgroLogo()
	{
		$upload_path=self::uploadPath();
		$logo1=getOptionA('website_logo1');
		if (!empty($logo)){
			if (file_exists($upload_path."/$logo1")){
				return uploadURL()."/$logo1";
			}
		} 
		return assetsURL()."/img/logo/logo1.png";
	}
    ...
    public static function getMenu2($class="menu")
    {
    	$top_menu_activated1=self::getTopMenuActivated1();
    
    	$top_menu[]=array('tag'=>"signup",'label'=>''.Yii::t("default","Home"),
                'url'=> websiteUrl() );
                                              
      $top_menu[]=array('tag'=>"grocery",
                'visible'=>in_array('browse',(array)$top_menu_activated1)?true:false,
                'label'=>''.Yii::t("default","Groceries"),
                'url'=>array('/store/grocery-search'));

     /* $top_menu[]=array('visible'=>in_array('browse',(array)$top_menu_activated1)?true:false,
        	'tag'=>'grocery','label'=>''.Yii::t("default",'Groceries'),                
             'itemOptions'=>array('class'=>''), 'items'=>array(
                   array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"fruit"),
                       'tag'=>'fruit', 'label'=>''.Yii::t("default","Fruit"),
                       'url'=>array('store/grocery-search')),        
              )); */       
      
        /*$top_menu[]=array('visible'=>in_array('browse',(array)$top_menu_activated)?true:false,
        'tag'=>'grocery',
                   'label'=>''.Yii::t("default",'Groceries'),
                   'itemOptions'=>array('class'=>''), 
                   'items'=>array(

                 array('visible'=>in_array('contact',(array)$top_menu_activated)?true:false,
                'tag'=>'contact','label'=>''.Yii::t("default","Contact"),
                'url'=>array('/store/product'));*/
        

         $top_menu[]=array('tag'=>"fruits&veg",
                'visible'=>in_array('browse',(array)$top_menu_activated1)?true:false,
                'label'=>''.Yii::t("default","Fruits & Vegetables"),
                'url'=>array('/store/product'));
               
        $top_menu[]=array('tag'=>"meal",
                'visible'=>in_array('contact',(array)$top_menu_activated1)?true:false,
                'label'=>''.Yii::t("default","Meal Plans"),
                'url'=>array('/store/product-details')); 

        $top_menu[]=array('tag'=>"sports",
                'visible'=>in_array('contact',(array)$top_menu_activated1)?true:false,
                'label'=>''.Yii::t("default","Sports & Fitness"),
                'url'=>array('/store/sport')); 

        $top_menu[]=array('tag'=>"wellness",
                'visible'=>in_array('contact',(array)$top_menu_activated1)?true:false,
                'label'=>''.Yii::t("default","Wellness"),
                'url'=>array('/store/welness'));                             
                                       
        $top_menu[]=array('tag'=>"drive",
                'visible'=>in_array('contact',(array)$top_menu_activated1)?true:false,
                'label'=>''.Yii::t("default","Drive Me"),
                'url'=>array('/store/driveme')); 
    ...
    public static function  searchGetFilter($getdata='')
    {
     ...
     /*public static function searchGrocery($params) {
     $query = mt_product_item::find();
    $dataProvider = new ActiveDataProvider([
     ...
     $dataProvider->setSort([
                    'product_item_name' => [
                'asc' => ['mt_product_item.product_item_name' => SORT_ASC],
                'desc' => ['mt_product_item.product_item_name' => SORT_DESC],
                'label' => 'Product Name'
            ]
        ]
    ]);
    ...
    if (!($this->load($params) && $this->validate())) {
        ...
        $query->joinWith(['mt_product_category']);
        return $dataProvider;
    }

    $this->addCondition($query, 'product_category_id');
    $this->addCondition($query, 'product_category_name', true);
        ...
        if (isset($_GET['product_category_name'])){
            $filter_cuisines=!empty($_GET['product_category_name'])?explode(",",$_GET['product_category_name']):false;
            if (is_array($filter_product) && count($filter_product)>=1){
                $x=1;
                foreach ($filter_product as $val) {                
                    if (!empty($val)){                      
                        if ( $x==1){
                            $filter_product.=" LIKE ".FunctionsV3::q('%"'.$val.'"%')." ";
                        } else $filter_product.=" OR product_category_name LIKE ".FunctionsV3::q('%"'.$val.'"%')." ";
                        
                        $x++;
                    }
                }               
                if (!empty($filter_product)){
                   $and.=" AND ($filter_product) ";
                }
            }
        }
               
        if (isset($_GET['product_item_name'])){
            if (!empty($_GET['product_item_name'])){              
                $and.=" AND product_item_name LIKE ".FunctionsV3::q("%".$_GET['product_item_name']."%")."  ";
            }
        }
...
/*	public static function getItemFirstPrice($prices='',$discount='')
    ...
    public static function getTopMenuActivated1()
    {
		$theme_top_menu=getOptionA('theme_top_menu');
		if(empty($theme_top_menu)){
			$theme_top_menu=array(
			  'grocery','fruit&veg','meal','sport','welness','driveme'
			);
		} else $theme_top_menu=json_decode($theme_top_menu,true);
    ...
    ///////////frontend get product items//////////////
    public static function getProductItem($product_item_id='',$shopping_category_id='')
    {
    	$DbExt=new DbExt;

    	$query="
    	SELECT pi.`product_item_name`, pi.`product_item_description`, pi.`regular_price`, pi.`sale_price`, pi.`photo`,  sc.`shopping_category_name`, pi.`weight`, pi.`length_measure` 
    	FROM mt_product_item pi
        JOIN mt_shopping_category sc 
        WHERE sc.`shopping_category_id` = pi.`shopping_category_id` 
    ...
    public static function getProductItemdemo1()
    {
    	...
    	SELECT pi.`product_item_name`, pi.`product_item_description`, pi.`regular_price`, pi.`sale_price`, pi.`photo`,  sc.`shopping_category_name`, pi.`weight`, pi.`length_measure` 
    	FROM mt_product_item pi 
        JOIN mt_shopping_category sc ON sc.`shopping_category_id` = pi.`shopping_category_id` 
    	ORDER BY pi.`product_item_id` ASC
    	";
    	//LIMIT 0,3";
    ...
    public static function getProductItemdemo2()
    {
    	...
    	$pagination = new Pagination(['totalCount' => $countQuery, 'defaultPageSize' => 5]);
    	$models = $query
    ...
    ///////////frontend get product items//////////////
     ///////////frontend get product items//////////////
    public static function getProductdisc($query='')
    {
    	$DbExt=new DbExt;
    	$query="
    	SELECT pi.`product_item_name`, pi.`product_item_description`, pi.`regular_price`, pi.`sale_price`, pi.`photo`,  sc.`shopping_category_name`, pi.`weight`, pi.`length_measure` 
    ...
    public static function getProductdisc1($query='',$shopping_category_id='')
    {
    	$DbExt=new DbExt;
    	$query="
    	SELECT pi.`product_item_name`, pi.`product_item_description`, pi.`regular_price`, pi.`sale_price`, pi.`photo`,  sc.`shopping_category_name`, pi.`weight`, pi.`length_measure` 
        ...
        WHERE sc.`shopping_category_id` = pi.`shopping_category_id`
    	ORDER BY pi.`product_item_id` ASC";
    	if (FunctionsV3::getProductItemdemo())
    	{
    	if ($res=$DbExt->rst($query)){
    		return $res;
    	}
