...
public function addShoppingCategory()
	{		
	   ...
	   $Validator=new Validator;
		$req=array(
		  'shopping_category_name'=>Yii::t("default","Shopping category name is required"),
		  'ShoppingCategoryImage'=>t("Icon is required")
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
			$params=array(
			  'shopping_category_name'=> $p->purify($this->data['shopping_category_name']) ,
			  'photo'=>$this->data['ShoppingCategoryImage'],
			  'status'=>$p->purify($this->data['status']),
			  'date_created'=>FunctionsV3::dateNow(),
			  'ip_address'=>$_SERVER['REMOTE_ADDR']
			);			
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{shopping_category}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetShoppingCateory($this->data['id'])){
						if($old_data['photo']!=$this->data['ShoppingCategoryImage']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{shopping_category}}' , $params ,'shopping_category_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Shopping category updated');  
...
/* shopping category list*/
	public function ShoppingCategoryList()
	{
	    ...
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[shopping_category_id]\" class=\"chk_child\" >";   		
	    		/*$option="<div class=\"options\">
	    		<a href=\"$slug/id/$val[dish_id]\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[dish_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";*/
	    		
	    		$slug=Yii::app()->createUrl('/admin/ShoppingCategory',array(
	    		  'Do'=>"Add",
	    		  'id'=>$val['shopping_category_id']
	    		));
	    		
	    		$option="<div class=\"options\">
	    		<a href=\"$slug\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[shopping_category_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";
	    		
	    		$date=FormatDateTime($val['date_created']);
	    		
	    		$feed_data['aaData'][]=array(
	    		  $val['shopping_category_id'],
	    		  $val['shopping_category_name'].$option,
	    		  '<img class="uk-thumbnail uk-thumbnail-mini" src="'.uploadURL()."/".$val['photo'].'">',
	    		  //$date."<div>".$val['status']."</div>"
	    		  "$date<br/><span class=\"tag ".$val['status']."\">".t($val['status'])."</span>",
	    		);
...
/* shopping category list*/
	...
	{		
	   ...
	   $Validator=new Validator;
		$req=array(
		  'product_category_name'=>Yii::t("default","Product category name is required"),
		  'ProductCategoryImage'=>t("Icon is required")
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
			$params=array(
			  'product_category_name'=> $p->purify($this->data['product_category_name']) ,
			  'shopping_category_id'=>$this->data['shopping_category_id'],
			  'photo'=>$this->data['ProductCategoryImage'],
			  'status'=>$p->purify($this->data['status']),
			  'date_created'=>FunctionsV3::dateNow(),
			  'ip_address'=>$_SERVER['REMOTE_ADDR']
			);			
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{product_category}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetProductCateory($this->data['id'])){
						if($old_data['photo']!=$this->data['ProductCategoryImage']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{product_category}}' , $params ,'product_category_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Product category updated');  
...
/* Product category list*/
	public function ProductCategoryList()
	{
		...
		INNER JOIN mt_shopping_category ON mt_product_category.`shopping_category_id`=mt_shopping_category.`shopping_category_id` 
	    ...
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[product_category_id]\" class=\"chk_child\" >";   		
	    		/*$option="<div class=\"options\">
	    		<a href=\"$slug/id/$val[dish_id]\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[dish_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";*/
	    		
	    		$slug=Yii::app()->createUrl('/admin/ProductCategory',array(
	    		  'Do'=>"Add",
	    		...
	    		$option="<div class=\"options\">
	    		<a href=\"$slug\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[product_category_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";
...
/* Add Product sub Category  */
	...
	{		
	   ...
	   $Validator=new Validator;
		$req=array(
		  'product_sub_category_name'=>Yii::t("default","Product sub category name is required"),
		  'ProductSubCategoryImage'=>t("Icon is required")
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
			$params=array(
			  'product_sub_category_name'=> $p->purify($this->data['product_sub_category_name']) ,
			  'shopping_category_id'=>$this->data['shopping_category_id'],
			  'product_category_id'=>$this->data['product_category_id'],
			  'photo'=>$this->data['ProductSubCategoryImage'],
			  'status'=>$p->purify($this->data['status']),
			  'date_created'=>FunctionsV3::dateNow(),
			  'ip_address'=>$_SERVER['REMOTE_ADDR']
			);			
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{product_sub_category}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetProductSubCategory($this->data['id'])){
						if($old_data['photo']!=$this->data['ProductSubCategoryImage']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{product_sub_category}}' , $params ,'product_sub_category_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Product sub category updated');  
...
/* Product sub category list*/
	public function ProductSubCategoryList()
	{
		...
		//$slug=$this->data['slug'];
        $stmt="
		SELECT * FROM mt_product_sub_category 
		JOIN mt_shopping_category ON mt_product_sub_category.`shopping_category_id`=mt_shopping_category.`shopping_category_id` 
		JOIN mt_product_category ON mt_product_sub_category.product_category_id = mt_product_category.product_category_id 
		ORDER BY mt_product_sub_category.product_category_id DESC;
		";
		$connection=Yii::app()->db;
	    $rows=$connection->createCommand($stmt)->queryAll();     	    
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[product_sub_category_id]\" class=\"chk_child\" >";   		
	    		/*$option="<div class=\"options\">
	    		<a href=\"$slug/id/$val[dish_id]\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[dish_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";*/
	    		
	    		$slug=Yii::app()->createUrl('/admin/ProductSubCategory',array(
	    		  'Do'=>"Add",
	    		  'id'=>$val['product_sub_category_id']
	    		));
	    		
	    		$option="<div class=\"options\">
	    		<a href=\"$slug\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[product_sub_category_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";
	    		
	    		$date=FormatDateTime($val['date_created']);
	    		
	    		$feed_data['aaData'][]=array(
	    		  $val['product_sub_category_id'],
	    		  $val['product_sub_category_name'].$option,
...
/* Add Product sub Category 4  */
	public function addProductSubCategory1()
	{		
	   ...
	   $Validator=new Validator;
		$req=array(
		  'product_sub_category1_name'=>Yii::t("default","Product sub category 4 name is required"),
		  'ProductSubCategory1Image'=>t("Icon is required")
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
			$params=array(
			  'product_sub_category1_name'=> $p->purify($this->data['product_sub_category1_name']) ,
			  'shopping_category_id'=>$this->data['shopping_category_id'],
			  'product_category_id'=>$this->data['product_category_id'],
			  'product_sub_category_id'=>$this->data['product_sub_category_id'],
			  'photo'=>$this->data['ProductSubCategory1Image'],
			  'status'=>$p->purify($this->data['status']),
			  'date_created'=>FunctionsV3::dateNow(),
			  'ip_address'=>$_SERVER['REMOTE_ADDR']
			);			
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{product_sub_category1}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetProductSubCateory1($this->data['id'])){
						if($old_data['photo']!=$this->data['ProductSubCategory1Image']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{product_sub_category1}}' , $params ,'product_sub_category1_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Product sub category 4 updated');  
...
/* Product sub category 4 list*/
	public function ProductSubCategory1List()
	{
				
        $stmt="
		SELECT psc1.product_sub_category1_id, psc1.product_sub_category1_name, psc1.photo, psc1.date_created, 
		psc1.status, psc.product_sub_category_name, sc.shopping_category_name, pc.product_category_name
		FROM mt_product_sub_category1 psc1
		JOIN mt_product_sub_category psc ON psc.product_sub_category_id = psc1.product_sub_category_id
		JOIN mt_shopping_category sc ON psc1.`shopping_category_id`=sc.`shopping_category_id` 
		JOIN mt_product_category pc ON psc1.product_category_id = pc.product_category_id 
		ORDER BY psc1.product_sub_category1_id DESC;
		";
		$connection=Yii::app()->db;
	    $rows=$connection->createCommand($stmt)->queryAll();     	    
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[product_sub_category1_id]\" class=\"chk_child\" >";   		
	    		/*$option="<div class=\"options\">
	    		<a href=\"$slug/id/$val[dish_id]\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[dish_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";*/
	    		
	    		$slug=Yii::app()->createUrl('/admin/ProductSubCategory1',array(
	    		  'Do'=>"Add",
	    		  'id'=>$val['product_sub_category1_id']
	    		));
	    		
	    		$option="<div class=\"options\">
	    		<a href=\"$slug\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[product_sub_category1_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";
	    		
	    		$date=FormatDateTime($val['date_created']);
	    		
	    		$feed_data['aaData'][]=array(
	    		  $val['product_sub_category1_id'],
	    		  $val['product_sub_category1_name'].$option,
	    		   $val['product_sub_category_name'],
	    		  $val['product_category_name'],
	    		  $val['shopping_category_name'],
	    		  '<img class="uk-thumbnail uk-thumbnail-mini" src="'.uploadURL()."/".$val['photo'].'">',
	    		  //$date."<div>".$val['status']."</div>"
...
/* Add Product sub Category 5  */
	public function addProductSubCategory2()
	{		
	   ...
	   $Validator=new Validator;
		$req=array(
		  'product_sub_category2_name'=>Yii::t("default","Product sub category 5 name is required"),
		  'ProductSubCategory2Image'=>t("Icon is required")
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
			$params=array(
			  'product_sub_category2_name'=> $p->purify($this->data['product_sub_category2_name']) ,
			  'shopping_category_id'=>$this->data['shopping_category_id'],
			  'product_category_id'=>$this->data['product_category_id'],
			  'product_sub_category_id'=>$this->data['product_sub_category_id'],
			  'product_sub_category1_id'=>$this->data['product_sub_category1_id'],
			  'photo'=>$this->data['ProductSubCategory2Image'],
			  'status'=>$p->purify($this->data['status']),
			  'date_created'=>FunctionsV3::dateNow(),
			  'ip_address'=>$_SERVER['REMOTE_ADDR']
			);			
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{product_sub_category2}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetProductSubCategory2($this->data['id'])){
						if($old_data['photo']!=$this->data['ProductSubCategory2Image']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{product_sub_category2}}' , $params ,'product_sub_category2_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Product sub category 5 updated');  
...
/* Product sub category 5 list*/
	public function ProductSubCategory2List()
	{
				
        $stmt="
		SELECT psc2.product_sub_category2_id, psc2.product_sub_category2_name, psc2.photo, psc2.date_created, 
		psc2.status,psc1.product_sub_category1_name, psc.product_sub_category_name, sc.shopping_category_name, pc.product_category_name
		FROM mt_product_sub_category2 psc2
        JOIN mt_product_sub_category1 psc1 ON psc1.product_sub_category1_id = psc2.product_sub_category1_id
		JOIN mt_product_sub_category psc ON psc.product_sub_category_id = psc1.product_sub_category_id
		JOIN mt_shopping_category sc ON psc1.`shopping_category_id`=sc.`shopping_category_id` 
		JOIN mt_product_category pc ON psc1.product_category_id = pc.product_category_id 
		ORDER BY psc1.product_sub_category1_id DESC;
	    ...
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[product_sub_category2_id]\" class=\"chk_child\" >";   		
	    		/*$option="<div class=\"options\">
	    		<a href=\"$slug/id/$val[dish_id]\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[dish_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";*/
	    		
	    		$slug=Yii::app()->createUrl('/admin/ProductSubCategory2',array(
	    		  'Do'=>"Add",
	    		  'id'=>$val['product_sub_category2_id']
	    		));
	    		
	    		$option="<div class=\"options\">
	    		<a href=\"$slug\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[product_sub_category2_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";
	    		
	    		$date=FormatDateTime($val['date_created']);
	    		
	    		$feed_data['aaData'][]=array(
	    		  $val['product_sub_category2_id'],
	    		  $val['product_sub_category2_name'].$option,
	    		  $val['product_sub_category1_name'],
	    		  $val['product_sub_category_name'],
	    		  $val['product_category_name'],
	    		  $val['shopping_category_name'],
	    		  '<img class="uk-thumbnail uk-thumbnail-mini" src="'.uploadURL()."/".$val['photo'].'">',
...
/* Add Product item  */
	...
	{		
	   ...
	   $Validator=new Validator;
		$req=array(
		  'product_item_name'=>Yii::t("default","Product item name is required"),
		  'FeaturedImage'=>t("Featured image is required"),
		  'product_item_description'=>Yii::t("default","Product description is required"),
		  'regular_price'=>Yii::t("default","Regular price is required"),
		  'sale_price'=>Yii::t("default","Sale price is required"),
		  //'multiple_upload'=>Yii::t("default","Gallery images is required"),
		  'quantity'=>Yii::t("default","Quantity is required"),
		  'stock_status'=>Yii::t("default","Stock status is required"),
		  'weight'=>Yii::t("default","Weight is required")
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
			$params=array(
			  'product_item_name'=> $p->purify($this->data['product_item_name']) ,
			  'shopping_category_id'=>$this->data['shopping_category_id'],
			  'product_category_id'=>$this->data['product_category_id'],
			  'product_sub_category_id'=>$this->data['product_sub_category_id'],
			  'product_sub_category4_id'=>$this->data['product_sub_category1_id'],
			  'product_sub_category5_id'=>$this->data['product_sub_category2_id'],
			  'product_item_description'=>$this->data['product_item_description'],
			  'wholesale_price'=>$this->data['wholesale_price'],
			  'regular_price'=>$this->data['regular_price'],
			  'sale_price'=>$this->data['sale_price'],
			  'featured_product'=>$this->data['featured_product'],
			  'photo'=>$this->data['FeaturedImage'],
			  'quantity'=>$this->data['quantity'],
			  'stock_status'=>$this->data['stock_status'],
			  'size'=>$this->data['size'],
			  'weight'=>$this->data['weight'],
			  'length_measure'=>$this->data['length'],
			  'dimensions'=>$this->data['dimensions'],
			  'special_tags'=>$this->data['special_tags'],
			  'shopping_category_link_products'=>json_encode($this->data['link_products_shopping_category']),
			  'product_category_link_products'=>json_encode($this->data['link_products_product_category']),
			  'product_sub_category_link_products'=>json_encode($this->data['link_products_product_sub_category']),
			  'product_sub_category4_link_products'=>json_encode($this->data['link_products_product_sub_category4']),
			  'product_sub_category5_link_products'=>json_encode($this->data['link_products_product_sub_category5']),
			  'product_item_link_products'=>json_encode($this->data['product_item_link_products']),
		   ...
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{product_item}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetProductItem($this->data['id'])){
						if($old_data['photo']!=$this->data['FeaturedImage']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{product_item}}' , $params ,'product_item_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Product item updated');  
...
/* Product item list */
	public function ProductItemList()
	{
		...
		//$slug=$this->data['slug'];
        $stmt="
		SELECT p.product_item_id, p.product_item_name, p.photo, s.shopping_category_name, pc.product_category_name,psc.product_sub_category_name, 
		p.date_created,p.status
		FROM mt_product_item p
		JOIN mt_shopping_category s ON p.`shopping_category_id`=s.`shopping_category_id` 
		JOIN mt_product_category pc ON p.product_category_id = pc.product_category_id 
		JOIN mt_product_sub_category psc ON psc.product_sub_category_id = p.product_sub_category_id 
		ORDER BY p.product_item_id DESC;";
		$connection=Yii::app()->db;
	    $rows=$connection->createCommand($stmt)->queryAll();     	    
	    if (is_array($rows) && count($rows)>=1){
	    	foreach ($rows as $val) {    	     	    		
	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[product_item_id]\" class=\"chk_child\" >";   		
	    
	    		$slug=Yii::app()->createUrl('/admin/ProductItem',array(
	    		  'Do'=>"Add",
	    		  'id'=>$val['product_item_id']
	    		));
	    		
	    		$option="<div class=\"options\">
	    		<a href=\"$slug\" >".Yii::t("default","Edit")."</a>
	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[product_item_id]\" >".Yii::t("default","Delete")."</a>
	    		</div>";
...
/////customised voucher//////////////
	...
	{					
		...
		$params=array(
		  ...
		  'product_sub_category4_link_products'=>json_encode($this->data['link_products_product_sub_category4']),
		  'product_sub_category5_link_products'=>json_encode($this->data['link_products_product_sub_category5']),
		  'product_Item_link_products'=>json_encode($this->data['link_products_product_item']),
		  'status'=>$this->data['status'],
		  'date_created'=>FunctionsV3::dateNow(),
		  'ip_address'=>$_SERVER['REMOTE_ADDR'],
...
/////admin add offers/////////////
		public function addOffersAdmin()
	    {
	    	$merchant_id=Yii::app()->functions->getMerchantID();
		    $params=array(
...
////////admin or merchant login identification///////////
			if (isset($this->data['voucher_owner'])){
			unset($params['merchant_id']);
			$params['voucher_owner']=$this->data['voucher_owner'];
			}
...
public function addCustomerProfile()
	{		
	   ...
	   $Validator=new Validator;
		$req=array(
		  'profile_name'=>Yii::t("default","Profile name is required"),
		  'saluation'=>t("Saluation is required"),
		  'first_name'=>t("First name is required"),
		  'last_name'=>t("Last name is required"),
		  //'dob'=>t("Date of birth is required"),
		  'gender'=>t("Gender is required"),
		  'email_id'=>t("Email address is required"),
		  'mobile_number'=>t("Mobile no is required"),
		  'street'=>t("Street is required"),
		  'city'=>t("City is required"),
		  'post_code'=>t("Post code is required"),
		  'state'=>t("State is required"),
		);		
		$Validator->required($req,$this->data);
		if ($Validator->validate()){
			$params=array(
			  'profile_name'=> $p->purify($this->data['profile_name']) ,
			  'saluation'=>$this->data['saluation'],
			  'first_name'=>$p->purify($this->data['first_name']),
			  'last_name'=>$p->purify($this->data['last_name']),
			  'dob'=>$p->purify($this->data['dob']),
			  'photo'=>$this->data['CustomerProfileImage'],
			  'gender'=>$p->purify($this->data['gender']),
			  'email_id'=>$p->purify($this->data['email_id']),
			  'mobile_number'=>$p->purify($this->data['mobile_number']),
			  'office_phone'=>$p->purify($this->data['office_phone']),
			  'home_phone'=>$p->purify($this->data['home_phone']),
			  'street'=>$p->purify($this->data['street']),
			  'city'=>$p->purify($this->data['city']),
			  'post_code'=>$p->purify($this->data['post_code']),
			  'state'=>$p->purify($this->data['state']),
			  'wedding_anniversary'=>$p->purify($this->data['wedding_anniversary']),
			  'languages'=>$p->purify($this->data['languages']),
			  'occupation'=>$p->purify($this->data['occupation']),
			  'employment_status'=>$p->purify($this->data['employment_status']),
			  'marital_status'=>$p->purify($this->data['marital_status']),
			  'employment_status'=>$p->purify($this->data['employment_status']),
			  'date_created'=>FunctionsV3::dateNow(),
			  'ip_address'=>$_SERVER['REMOTE_ADDR']
			);			
		   if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{customer_profile}}",$params)){
		    		    $this->details=Yii::app()->db->getLastInsertID();
			    		$this->code=1;
			    		$this->msg=Yii::t("default","Successful");				    		
			    	}
					...
					$filename_to_delete='';
					if($old_data=Yii::app()->functions->GetCustomerProfile($this->data['id'])){
						if($old_data['photo']!=$this->data['CustomerProfileImage']){
						   $filename_to_delete=$old_data['photo'];			
						}			
					}
					
					$res = $this->updateData('{{customer_profile}}' , $params ,'profile_id',$this->data['id']);
					if ($res){
						$this->code=1;
		                $this->msg=Yii::t("default",'Customer profile updated');  
