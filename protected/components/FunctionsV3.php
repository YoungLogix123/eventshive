...
public static function getProductItemdemo($page='')
    {
        $DbExt=new DbExt;
        $number_of_result= 3;
        $this_first_number=($page-1) * $number_of_result;
        $query="
        SELECT pi.`product_item_name`, pi.`product_item_description`, pi.`regular_price`, pi.`sale_price`, pi.`photo`,  sc.`shopping_category_name`, pi.`weight`, pi.`length_measure` 
        FROM mt_product_item pi
        JOIN mt_shopping_category sc ON sc.`shopping_category_id` = pi.`shopping_category_id` 
        ORDER BY pi.`product_item_id` ASC LIMIT ".$this_first_number.",".$number_of_result."";
        if ($res=$DbExt->rst($query)){
            return $res;
        }
        return false;
    ...
    public static function number_of_page(){
        $DbExt=new DbExt;
        $query="
        SELECT pi.`product_item_name`, pi.`product_item_description`, pi.`regular_price`, pi.`sale_price`, pi.`photo`,  sc.`shopping_category_name`, pi.`weight`, pi.`length_measure` 
        FROM mt_product_item pi
