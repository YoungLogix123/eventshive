...
{	
	...
	//multiple select product item//
	 public function ProductItemLinkProducts($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
    	{{product_item}}
    	ORDER BY product_item_name ASC
    	";
    	if ( $with_select){
    	...
    	if ($res=$DbExt->rst($stmt)){    		
    		if ( $as_list==TRUE){
    			foreach ($res as $val) {    				
    			    $data[$val['product_item_id']]=ucwords(stripslashes($val['product_item_name']));
    			}
    			return $data;
    		} else return $res;    	
    	}
	...
	//multiple select shopping category//
	 public function LinkProductsShoppingCategory($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
    	{{shopping_category}}
    	ORDER BY shopping_category_name ASC
    	";
    	if ( $with_select){
    		$data[]=t("Please select");
    	}
    	if ($res=$DbExt->rst($stmt)){    		
    		if ( $as_list==TRUE){
    			foreach ($res as $val) {    				
    			    $data[$val['shopping_category_id']]=ucwords(stripslashes($val['shopping_category_name']));
    			}
    			return $data;
    		} else return $res;    	
    	}
	...
	//multiple select product category//
	 public function LinkProductsProductCategory($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
    	...
    	if ($res=$DbExt->rst($stmt)){    		
    		if ( $as_list==TRUE){
    			foreach ($res as $val) {    				
    			    $data[$val['product_category_id']]=ucwords(stripslashes($val['product_category_name']));
    			}
    			return $data;
    		} else return $res;    	
    	}
	...
	//multiple select product sub category//
	 public function LinkProductsProductSubCategory($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
    	...
    	if ($res=$DbExt->rst($stmt)){    		
    		if ( $as_list==TRUE){
    			foreach ($res as $val) {    				
    			    $data[$val['product_sub_category_id']]=ucwords(stripslashes($val['product_sub_category_name']));
    			}
    			return $data;
    		} else return $res;    	
    	}
	...
	//multiple select product sub category 4//
	 public function LinkProductsProductSubCategory4($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
    	{{product_sub_category1}}
    	ORDER BY product_sub_category1_name ASC
    	";
    	if ( $with_select){
    		$data[]=t("Please select");
    	}
    	if ($res=$DbExt->rst($stmt)){    		
    		if ( $as_list==TRUE){
    			foreach ($res as $val) {    				
    			    $data[$val['product_sub_category1_id']]=ucwords(stripslashes($val['product_sub_category1_name']));
    			}
    			return $data;
    		} else return $res;    	
    	}
	...
	//multiple select product sub category 5//
	 public function LinkProductsProductSubCategory5($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
    	{{product_sub_category2}}
    	ORDER BY product_sub_category2_name ASC
    	";
    	if ( $with_select){
    		$data[]=t("Please select");
    	}
    	if ($res=$DbExt->rst($stmt)){    		
    		if ( $as_list==TRUE){
    			foreach ($res as $val) {    				
    			    $data[$val['product_sub_category2_id']]=ucwords(stripslashes($val['product_sub_category2_name']));
    			}
    			return $data;
    		} else return $res;    	
    	}
	...
	public function merchantMenu()
	{
		...
		return array(  
		    ...
		    'items'=>array(
                ...
                array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"CategoryList"),'tag'=>"CategoryList",'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Product Category"),
                'url'=>array('/merchant/CategoryList')),
                
                array('visible'=>FunctionsV3::hasMerchantAccessToMenu($user_access,"category_sked"),'tag'=>"category_sked",'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Product Category Scheduler"),
	...
	public function adminMenu($val='')	
	{
		switch ($val) {
			...
			case "dashboard":
			    ...
			    'items'=>array(
	                ...
	                'tag'=>"OrderStatus",'label'=>Yii::t("default","Order Status"),
	                'url'=>array('/admin/OrderStatus'), 'id'=>"ddddd"),
	                  	

					array('visible'=>$this->AA('incomingorders'),'tag'=>"incomingorders",
	                'label'=>Yii::t("default","All Orders"),
	                'url'=>array('/admin/incomingorders')), 
	 
					array('visible'=>$this->AA('cancel_order'),'tag'=>"cancel_order",
	                'label'=>Yii::t("default","New Cancel Orders"),
                ...
                array('visible'=>$this->AA('addonexport'),
                   'tag'=>'addonexport','label'=>'<i class="fa fa fa-list-alt"></i>'.Yii::t("default","Export/Import"), 
                   'url'=>Yii::app()->getBaseUrl(true)."/exportmanager?lang=".Yii::app()->language),

                array('visible'=>$this->AA('pointsprogram'),
                   'tag'=>'pointsprogram','label'=>'<i class="fa fa fa-list-alt"></i>'.Yii::t("default","Loyalty Points Program"), 
                   'url'=>Yii::app()->getBaseUrl(true)."/pointsprogram?lang=".Yii::app()->language), 

                array('visible'=>$this->AA('dishes'),
                'tag'=>"dishes",'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Inventory"),
                'url'=>array('/admin/inventory')),
            	
			    ));
					# code...
					break;

				case 'offerandpromotions':
				return array(  
			    'activeCssClass'=>'active', 
			    'encodeLabel'=>false,
			    'items'=>array(
                ...
                'tag'=>"voucher",
                'label'=>Yii::t("default","Voucher"),
                'url'=>array('/admin/voucher')),  
 
				array('visible'=>$this->AA('offers'),
                'tag'=>"offers",
                'label'=>Yii::t("default","Offers"),
                'url'=>array('/admin/offers')),  
                 )),
			    ...
			    'items'=>array(
			       array('items'=>array(
                	...
                	'label'=>Yii::t("default","Reviews"),
                	'url'=>array('admin/reviews')),

                   	array('visible'=>$this->AA('Ratings'),
                	'tag'=>"Ratings",'label'=>Yii::t("default","Ratings"),
                ...
                'tag'=>"emailtpl",'label'=>Yii::t("default","Notification Template"),
                'url'=>array('/admin/emailtpl')),           
                
                array('visible'=>$this->AA('notisettings'),
                'tag'=>"notisettings",'label'=>Yii::t("default","Notification Settings"),
			    ...
			    'items'=>array(	
                 ...
                 'tag'=>'printermodule','label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Printer"), 
			    ...
			    'items'=>array(
                   ...
                   'tag'=>'mobileappv2','label'=>Yii::t("default","MobileApp v2"), 
                   'url'=>Yii::app()->getBaseUrl(true)."/mobileappv2?lang=".Yii::app()->language),   

                   array('visible'=>$this->AA('merchantapp'),
                   'tag'=>'merchantapp','label'=>Yii::t("default","MerchantApp"), 
                   ...
                   'tag'=>'singlemerchant','label'=>Yii::t("default","Single Merchant"), 
                   'url'=>Yii::app()->getBaseUrl(true)."/singlemerchant?lang=".Yii::app()->language),   
                         
                    array('visible'=>$this->AA('merchantapp'),
                   'tag'=>'merchantapp','label'=>Yii::t("default","Driver"), 
                   'url'=>Yii::app()->getBaseUrl(true)."/merchantapp?lang=".Yii::app()->language),   
			    ...
			    'encodeLabel'=>false,
			    'items'=>array(
				array('visible'=>$this->AA('customerProfile'),
                'tag'=>"customerProfile",'label'=>'<i class="fa fa-users"></i>'.Yii::t("default","Customer Profile"),
                'url'=>array('/admin/customerProfile')),
				));
					# code...
					break;
				...
				case 'Grocery_Product_Categories':
				return array(  
			    'activeCssClass'=>'active', 
			    'encodeLabel'=>false,
			    'items'=>array(
			    array('items'=>array( 
				array('visible'=>$this->AA('ShoppingCategory'),'tag'=>'ShoppingCategory',
                     'label'=>Yii::t("default","Shopping Category"), 
                     'url'=>array('admin/ShoppingCategory')),   

                     array('visible'=>$this->AA('ProductCategory'),'tag'=>'ProductCategory',
                      'label'=>Yii::t("default","Product Category"), 
                     'url'=>array('admin/ProductCategory')),                              
                                                        
                     array('visible'=>$this->AA('ProductSubCategory'),'tag'=>'ProductSubCategory',
                      'label'=>Yii::t("default","Product Sub Category"), 
                     'url'=>array('admin/ProductSubCategory')),

                     array('visible'=>$this->AA('ProductSubCategory1'),'tag'=>'ProductSubCategory1',
                      'label'=>Yii::t("default","Product Sub Category 4"), 
                     'url'=>array('admin/ProductSubCategory1')),

                     array('visible'=>$this->AA('ProductSubCategory2'),'tag'=>'ProductSubCategory2',
                      'label'=>Yii::t("default","Product Sub Category 5"), 
                     'url'=>array('admin/ProductSubCategory2')),
                     
                     array('visible'=>$this->AA('ProductItem'),'tag'=>'ProductItem',
                      'label'=>Yii::t("default","Product Item"), 
                     'url'=>array('admin/ProductItem')),                                   
                 	)),  
				));
					# code...
					break;
		...
		/*return array(  
		    ...
		    'items'=>array(
                ...
                'tag'=>"packages",'label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","Packages"),
                'url'=>array('/admin/packages')), 

		    	array('visible'=>$this->AA('orders'),'tag'=>'orders',
                   'label'=>'<i class="fa fa-plus-square"></i>'.Yii::t("default",'Orders'),
                   'itemOptions'=>array('class'=>''), 
                   'items'=>array(

                 array('visible'=>$this->AA('OrderStatus'),
                'tag'=>"OrderStatus",'label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","Order Status"),
                'url'=>array('/admin/OrderStatus')),
                  	

				array('visible'=>$this->AA('incomingorders'),'tag'=>"incomingorders",
                'label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","All Orders"),
                'url'=>array('/admin/incomingorders')), 
 
				array('visible'=>$this->AA('cancel_order'),'tag'=>"cancel_order",
                'label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","New Cancel Orders"),
                'url'=>array('/admin/cancel_order')), 
				)),

				   //////Payments/////////////////  
				array('visible'=>$this->AA('payments'),
                 'tag'=>'payments','label'=>'<i class="fa fa-plus-square"></i>'.Yii::t("default",'Finance'),
                   'itemOptions'=>array('class'=>''), 'items'=>array(  
                ...
                'url'=>array('/admin/invoice')), 
                   ...
                   array('visible'=>$this->AA('bankdeposit'),'tag'=>"bankdeposit",
                'label'=>'<i class="fa fa-paypal"></i>'.Yii::t("default","Receive Bank Deposit"),
                'url'=>array('/admin/bankdeposit')),  
...
'sub_item_name',$addon_name_trans).'</div>';
    ...
    // shopping category dropdown ////

    public function shoppigCategoryList($as_list=true,$with_select=true)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
   ...
   // product category dropdown ////
    ...
    public function productCategoryList($as_list=true,$with_select=true)
    {

   /*if (Yii::app()->request->isAjaxRequest)
    {
    	$shopping_category_id = Yii::app()->request->getParam( 'shopping_category_id' );

    	echo $shopping_category_id;
    ...
    public function productSubCategoryList($as_list=true,$with_select=true)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
  ...
  // Product sub category 4 dropdown ////
    public function productSubCategory4List($as_list=true,$with_select=true)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
     ...
     // Product sub category 4 dropdown ////
      ...
      // Product sub category 5 dropdown ////
    public function productSubCategory5List($as_list=true,$with_select=true)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
     ...
     // Product sub category 1 dropdown (sub category 4) ////
    public function productSubCategory1List($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
     ...
     // Product sub category dropdown (sub category 4)  ////

    // Size dropdown ////

    public function sizeDropdown($as_list=true,$with_select=false)
    {
    	$data=array();
    	$DbExt=new DbExt;
    	$stmt="SELECT * FROM
    	{{size}}
    	WHERE status in ('publish') and merchant_id = '1'
    	...
    	if ($res=$DbExt->rst($stmt)){    		
    		if ( $as_list==TRUE){
    			foreach ($res as $val) {    				
    			    $data[$val['size_id']]=ucwords(stripslashes($val['size_name']));
    			}
    			return $data;
    		} else return $res;    	
    	}
...
/* get shopping category*/
	public function GetShoppingCateory($id='')
	{
		$DbExt=new DbExt;
		$stmt="SELECT * FROM
		{{shopping_category}}
		WHERE
		shopping_category_id= ".FunctionsV3::q($id)."
		LIMIT 0,1
		";
		if ($res=$DbExt->rst($stmt)){
			return $res[0];
...
/* get product category*/
	public function GetProductCateory($id='')
	{
		$DbExt=new DbExt;
		$stmt="SELECT * FROM
		{{product_category}}
		WHERE
		product_category_id= ".FunctionsV3::q($id)."
		LIMIT 0,1
		";
		if ($res=$DbExt->rst($stmt)){
			return $res[0];
...
/* get products sub category*/
	public function GetProductSubCategory($id='')
	{
		...
		{{product_sub_category}}
		WHERE
		product_sub_category_id= ".FunctionsV3::q($id)."
		LIMIT 0,1
		";
		if ($res=$DbExt->rst($stmt)){
			return $res[0];
...
/* get products sub category 1*/
	public function GetProductSubCateory1($id='')
	{
		$DbExt=new DbExt;
		$stmt="SELECT * FROM
		{{product_sub_category1}}
		WHERE
		product_sub_category1_id= ".FunctionsV3::q($id)."
		LIMIT 0,1
		";
		if ($res=$DbExt->rst($stmt)){
			return $res[0];
...
/* get product sub category 1*/

/* get products sub category 2*/
	public function GetProductSubCategory2($id='')
	{
		$DbExt=new DbExt;
		$stmt="SELECT * FROM
		{{product_sub_category2}}
		WHERE
		product_sub_category2_id= ".FunctionsV3::q($id)."
		LIMIT 0,1
		";
		if ($res=$DbExt->rst($stmt)){
			return $res[0];
...
/* get products items*/
	public function GetProductItem($id='')
	{
		...
		{{product_item}}
		WHERE
		product_item_id= ".FunctionsV3::q($id)."
		LIMIT 0,1
		";
		if ($res=$DbExt->rst($stmt)){
			return $res[0];
