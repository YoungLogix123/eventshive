...
var notification_interval = 33000;
...
jQuery(document).ready(function() {
   ...
   $('.is_ready').on('ifUnchecked', function(event){    	
	...
	if ( $("#currentController").val()=="merchant" ){
	 	new_order_notify = setInterval(function(){get_new_order()}, notification_interval );
	}	
	//}
	
	if ( $("#booking_alert").exists() ){				
		if ( $("#booking_alert").val()=="" ){				
		 	new_order_notify = setInterval(function(){get_booking()}, notification_interval);
		}
	}
...
jQuery(document).ready(function() {	
    ...
    if ( $("#wd_payout_alert").exists() ){
    	...
    	if ( wd_payout_alert=="2"){    		
    		new_order_notify = setInterval(function(){wdPayoutNotification()}, notification_interval);
    	}
    }    
...
jQuery(document).ready(function() {	
	...
	if ( $("#currentController").val()=="admin" ){
		if (order_notification!=1){
			 new_order_notify = setInterval(function(){getAdminNewOrder()}, notification_interval);					 			 
		}		
	}
...
jQuery(document).ready(function() {
	...
	if ( current_panel =="admin" ){
		if(!empty(has_session)){
			if(has_session==1){
		 	   admin_cancel_order_handle = setInterval(function(){getNewCancelOrderAdmin()}, notification_interval);
			}
		}
	}	
...
mapbox_init_map = function(lat, lng){
	if(!empty(lat) && !empty(lng)){
		...
		if(empty(mapbox_handle)){
			...
			L.tileLayer('https://api.tiles.mapbox.com/v4/{id}/{z}/{x}/{y}.png?access_token='+map_apikey, {		    
			    maxZoom: 18,
			    id: 'mapbox.streets',		    
			}).addTo(mapbox_handle);
